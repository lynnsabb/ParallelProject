...
void *compute_correlation_thread(void *arg) {
    ...
    int end_row = (args->thread_id == args->num_threads - 1) ? 
                  ds->num_features : (args->thread_id + 1) * features_per_thread;

    for (int i = start_row; i < end_row; i++) {
        for (int j = 0; j < ds->num_features; j++) {
            args->corr_matrix[i][j] = compute_correlation(
...
void perform_analysis(Dataset *ds, int num_threads) {
    ...
    printf("Computing correlation matrix with %d threads...\n", num_threads);
    parallel_correlation_matrix(ds, corr_matrix, num_threads);
...
int main(int argc, char *argv[]) {
    if (argc < 3) {
        printf("Usage: %s <dataset.csv> <num_threads>\n", argv[0]);
        return 1;
    }
    ...
    clock_t start = clock();
    perform_analysis(&ds, num_threads);
    clock_t end = clock();

    double cpu_time = ((double)(end - start)) / CLOCKS_PER_SEC;
    printf("\n=== Performance ===\n");
    printf("Pthreads execution time (%d threads): %.4f seconds\n", num_threads, cpu_time);
